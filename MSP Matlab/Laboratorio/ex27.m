...
%% resampling without matlab functions, using a filter with 64 samples
...
norm_freq_axis = 0:1/N:1 - 1/N;

figure; 
leg = {};
stem(norm_freq_axis, abs(Xf));
leg{1} = 'Original signal';
hold on;
stem(norm_freq_axis, abs(Xup));
leg{2} = 'Upsampled signal';
hold on;
stem(norm_freq_axis, abs(Xfilt), '*-');
leg{3} = 'Filtered signal';
hold on, 
stem(norm_freq_axis, abs(Yf), 'o-');
