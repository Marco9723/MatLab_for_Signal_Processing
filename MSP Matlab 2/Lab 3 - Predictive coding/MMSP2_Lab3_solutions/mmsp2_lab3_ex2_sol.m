...
for ii = 1:length(R)
    ...
    % next samples: DPCM
    d_l_aux = xl(2:end) - xl(1:end-1); % without quantizer, this would be the residual
    ...
    % Let's build a PCM codec, just for comparison 
...
figure
plot(R,SNR_l,R,SNR_r,R,SNR_d,R, SNR_pcm, 'linewidth', 2);
